...
| 문제 | 고민 | 해결 |
|---|---|---|
| **신뢰 경계** — 기존 Firebase 정적 구조는 가격·재고·쿠폰을 클라이언트가 다뤄 주문 금액을 조작할 여지 | 어디서 값을 확정해야 위변조를 막나 | 주문 생성을 **`place_order` RPC(security definer) 한 곳**으로 모아 가격·재고·쿠폰을 모두 DB에서 확정 — 클라 값은 신뢰하지 않음 |
| **동시성** — 주문이 몰리면 재고가 음수가 되거나 쿠폰이 중복 사용 | 경쟁 상태(race condition)를 어떻게 막나 | RPC 내부에서 `for update` **행잠금 + 단일 트랜잭션**으로 재고 차감·쿠폰 사용을 원자 처리 (실패 시 전체 롤백) |
| **권한·위조** — 일반 유저가 role/포인트를 바꾸거나, 사지 않은 상품에 인증 리뷰를 다는 위조 | 접근 제어를 코드가 아닌 DB가 강제하게 | **RLS 53개 정책 + 트리거** — `protect_profile_fields`(권한 필드 변경 차단), `set_review_verified`(구매 이력 기반 인증) |
