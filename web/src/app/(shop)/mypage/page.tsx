...
export default async function MypageHome() {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* 프로필 카드 */}
      <section className="border border-sand bg-sand/30 p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <span className="inline-block border border-antique-gold px-3 py-1 text-xs uppercase tracking-[0.15em] text-antique-gold">
      ...
      {/* 적립금 */}
      <section className="flex items-center justify-between border border-sand p-6">
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {[
          ...
          <Link
            key={l.href}
            href={l.href}
            className="flex h-20 items-center justify-center border border-sand text-sm text-muted-foreground transition-colors hover:border-dark hover:text-dark"
          >
            {l.label}
          </Link>
        ))}
